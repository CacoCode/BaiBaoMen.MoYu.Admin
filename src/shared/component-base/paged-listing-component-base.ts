...
export abstract class PagedListingComponentBase<EntityDto>
  ...
  exportList() {
    ...
    this.exportDataList(request, () => {

    });
  }

  downExcelFile(file: FileDto) {
    const url = AppConsts.remoteServiceBaseUrl + '/api/services/client/File/DownloadTempFile?fileType=' + file.fileType + '&fileToken=' + file.fileToken + '&fileName=' + file.fileName;
   ...
   * @param finishedCallback 完成后回调函数
   */
  protected abstract exportDataList(
    request: PagedRequestDto,
    finishedCallback?: Function,
  ): void;
}
