...
import {
  EditionEditDto, ComboboxItemDto, EditionServiceProxy, CommonLookupServiceProxy,
  SubscribableEditionComboboxItemDto, CreateOrUpdateEditionDto
...
export class CreateOrEditEditionComponent extends ModalComponentBase implements OnInit {
  ...
  @ViewChild('featureTree', { read: '', static: false }) featureTree: EditionFeatureTreeComponent;
  ...
  show(editionId?: number): void {
    ...
    this._commonLookupService.getEditionsForCombobox(true).subscribe(editionsResult => {
      ...
      this._editionService.getEditionForEdit(editionId).subscribe(editionResult => {
        ...
        this.expireAction = this.edition.expiringEditionId > 0 ? AppEditionExpireAction.AssignToAnotherEdition :
